...
const WhiteBox = props => {
  return (
    <div className={props.className} className="bg-white rounded-md shadow-lg">
      <div className="px-4 py-2.5 rounded-md shadow-sm overflow-hidden">
        <p className="text-xs font-extralight leading-normal text-black">
          {props.children}
        </p>
      </div>
    </div>
    ...
    >
      <div className="flex">
        <div className="flex-shrink-0">
          {props.type === "warning" ? (
            <WarnIcon className="h-5 w-5 text-yellow-400" />
          ) : (
            ``
          )}
          {props.type === "error" ? (
            <AlertIcon className="h-5 w-5 text-red-400 fill-current" />
          ) : (
            ``
          )}
          {props.type === "success" ? (
            <CheckIcon className="h-5 w-5 fill-current text-green-400" />
          ) : (
            ``
          )}
          {props.type === "general" ? (
            <InfoIcon className="h-5 w-5 fill-current text-blue-400" />
          ) : (
            ``
          )}
        </div>
