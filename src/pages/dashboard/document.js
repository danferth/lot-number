...
const CertificateRequest = ({ data }) => {
  ...
  return (
    <div tw="bg-gray-700 min-h-screen">
      <div tw="py-8 print:hidden bg-gray-100">
        <div tw="flex items-center justify-start max-w-4xl mx-auto px-4">
          <Button
            type="button"
            onClick={() => {
              window.print()
          ...
          <p tw="text-gray-500 font-light text-sm ml-8">
      ...
      {documentReady && !documentError && (
        <div tw="max-w-5xl mx-auto mt-8 mb-4">
          ...
          {docType.current === "sterilizationprocess" &&
            !productData.current.sterile && (
              <p tw="text-white mx-auto text-lg">
...
export const query = graphql`
  {
    products: allQcProductsJson(filter: {}) {
      edges {
        node {
          id
          title
